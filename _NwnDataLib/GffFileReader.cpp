...
//			NULL);
		...
		if (File == nullptr)
			throw std::runtime_error( "Failed to open GFF file." );
	}
	...
	try
	{
// TODO		m_FileSize = GetFileSize( File, NULL );

// TODO		if ((m_FileSize == 0xFFFFFFFF) && (GetLastError( ) != NO_ERROR))
...
// TODO		CloseHandle( File );
		...
		m_FileWrapper.SetFileHandle( nullptr );
...
--*/
{
	...
	NameLen = strlen( Name );
	NameLen = vsmin( NameLen, sizeof( LabelEntry.Name ) );
