...
public:
	...
	StackPushEngineStructure(
		 EngineStructurePtr Struct
		) = 0;

	virtual
	EngineStructurePtr
	StackPopEngineStructure(
		 ENGINE_STRUCTURE_NUMBER EngineType
		) = 0;
...
public:
	...
	class type_mismatch_exception   : public std::runtime_error { public: type_mismatch_exception(  const char * s ) : std::runtime_error( s ) {} };
	class stack_underflow_exception : public std::runtime_error { public: stack_underflow_exception(  const char * s ) : std::runtime_error( s ) {} };
	class stack_overflow_exception  : public std::runtime_error { public: stack_overflow_exception(  const char * s ) : std::runtime_error( s ) {} };
	class invalid_handle_exception  : public std::runtime_error { public: invalid_handle_exception(  const char * s ) : std::runtime_error( s ) {} };
	class invalid_stack_exception   : public std::runtime_error { public: invalid_stack_exception(  const char * s ) : std::runtime_error( s ) {} };
	class guard_zone_exception      : public std::runtime_error { public: guard_zone_exception(  const char * s ) : std::runtime_error( s ) {} };
	...
	NWScriptStack(
		 NWN::OBJECTID InvalidObjId = NWN::INVALIDOBJID
		);
	...
	void
	CopyDownSP(
		 STACK_POINTER Destination,
		 STACK_POINTER BytesToCopy,
		 bool UseBP = false
		);
	...
	NWScriptStack
	SaveStack(
		 STACK_POINTER BPSaveBytes,
		 STACK_POINTER SPSaveBytes,
		 STACK_POINTER SPSaveOffset = 0
		);
	...
	void
	DestructElements(
		 STACK_POINTER BytesToRemove,
		 STACK_POINTER ExcludePointer,
		 STACK_POINTER BytesToExclude
		);
...
private:
	...
	StackPushRaw(
		 ...
		 STACK_TYPE_CODE StackEntryType
		);
